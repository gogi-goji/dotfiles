...
#} 2> /dev/null | {
    ...
    while true ; do
        ...
        # The following are for date and time and stuff, so they shouldn't be needed here (on right maybe?)    
        # small adjustments
...
#} 2> /dev/null | dzen2 -w $panel_width -x $x_left -y $y_position -fn "$font" -h $panel_height \
#    -ta l -bg "$bgcolor" -fg "$fgcolor"
} | dzen2 -w $panel_width -x $x_left -y $y_position -fn "$font" -h $panel_height \
    -ta l -bg "$bgcolor" -fg "$fgcolor" -p
